# This file generates synthetic networks across varying rho (the RGG networks are assortative)
# An output folder should be made in the same directory and the helper file should be in the same directory
#
...
OUTPUT_FOLDER = 'Synth_Nets_Directed_Assortative'
...
def step_fn_kernel_assortative(distance):
    if distance < 1.00375:
        prob = 1
    else:
        prob = 0
...
for s in range(0,N):
    ...
    nets_attr_curr = []
    for i in range(0,M):
        net_struc = generate_helper.mixed_network_directed(NODES, NUM_BIN, NUM_NUM, NUM_RANGES, BLOCKZ, PROBZ, step_fn_kernel_assortative, REC_PROP, rho_struc, None)
        net_full = generate_helper.mixed_network_directed(NODES, NUM_BIN, NUM_NUM, NUM_RANGES, BLOCKZ, PROBZ, step_fn_kernel_assortative, REC_PROP, rho_full, None)
        net_attr = generate_helper.mixed_network_directed(NODES, NUM_BIN, NUM_NUM, NUM_RANGES, BLOCKZ, PROBZ, step_fn_kernel_assortative, REC_PROP, rho_attr, None)
        nets_struc_curr.append(net_struc)
        nets_full_curr.append(net_full)
        nets_attr_curr.append(net_attr)
    auc_max_curr = generate_helper.get_theoretical_auc_max(nets_full_curr,PROBZ,step_fn_kernel_assortative,rho_full,REC_PROP)
